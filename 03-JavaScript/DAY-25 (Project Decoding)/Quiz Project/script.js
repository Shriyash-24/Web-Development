...
function RandomQuestion(){
    ...
    // const data = new Set(); // use set for unique objects.
    // while(data.size!=5){
    ...
    // Another Approach
    // sort() --> Works differently. It converts numbers to strings and compares strings. It comapres 2 things at once.
    // Sorting the questionBank array randomly.
    
    // questionBank.sort(()=>Math.random()-0.5); // -0.5 isiliye kiya hai because kuch value -ve and kuch value +ve hoo jayegi and that will sort it out.
...
problem.forEach((obj,index)=>{
    const div_element = document.createElement('div'); // creating div.
    div_element.className = 'question'; // class of div will be question now.
    original_answer[`q${index+1}`]= obj['answer'];
    const para = document.createElement('p'); // made a para.
    para.textContent = `${index+1}.${obj['question']}`; // Adding a value to question along with accessing the question.
    div_element.appendChild(para); // appending the para in the div as a child.

    // Accessing options array and applying forEach.
    obj['options'].forEach((data)=>{
        ...
        label.appendChild(input); // input is appended as a child of label.
        label.appendChild(document.createTextNode(data)); // Adding text node.
        div_element.appendChild(label); // div mei label to append kar diya.
        div_element.appendChild(document.createElement('br')); // Now br also adds up
    })
